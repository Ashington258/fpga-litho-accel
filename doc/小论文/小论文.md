...
- 提出一种面向计算光刻 TCC-SOCS 在线空中像重构的 CPU-FPGA 协同加速框架，将光学参数变化时执行的离线预处理与面向掩模窗口频繁执行的在线重构解耦，在保持模型精度和配置灵活性的同时降低在线计算延迟并提升能效。
...
# 面向部分相干光刻空中像计算的低延迟高能效 FPGA 加速架构

## 摘要


基于传输交叉系数（transmission cross coefficient，TCC）和相干系统求和（sum of coherent systems，SOCS）的在线空中像重构是 OPC、SMO 和 ILT 等计算光刻流程中频繁执行的计算瓶颈，传统 CPU 平台难以在低功耗部署场景下兼顾计算延迟与能效。针对这一问题，本文提出一种 CPU-FPGA 协同分离式加速架构：CPU 负责离线 TCC 构建与 SOCS 本征核提取，FPGA 负责在线频域嵌入、二维 IFFT 和加权累加；通过固定 FFT 网格、块浮点补偿、多路 AXI-MM 接口和 BRAM 缓冲构建面积高效的数据通路。实验结果表明，相较执行相同在线重构算子的优化 C++ SOCS 基准，所提出 FPGA 内核实现 3.37 倍加速；基于当前功耗估计，其内核级能效提升约 67.4 倍，验证了该架构面向低功耗计算光刻在线重构的有效性。

**关键词：**计算光刻；TCC-SOCS；CPU-FPGA 协同；FPGA 加速；高能效计算

## 1. 引言

计算光刻已成为先进半导体制造中连接光学成像、工艺窗口和版图优化的关键技术。随着工艺节点持续缩小，单纯依靠投影光学系统提升分辨率愈发困难，光学邻近效应校正（OPC）、源掩模协同优化（SMO）和反向光刻技术（ILT）等方法被广泛用于补偿邻近效应、优化照明与掩模形状并扩大可制造窗口 [1]-[11]。这些方法通常以模型反馈或反问题优化的形式工作，需要在大量版图窗口、多种工艺条件和多轮迭代中频繁调用空中像计算 [2], [3], [5], [8]。当工艺窗口缩小、掩模复杂度提高且优化变量从边缘移动扩展到像素化或曲线图形时，空中像计算的延迟、吞吐和能耗逐渐成为计算光刻工程部署中的核心瓶颈。
...
SOCS 将直接 TCC 中的密集频率耦合转换为有限数量相干系统的加权求和，离线分解之后的在线重构阶段仍是频繁执行的计算热点。对于每个 SOCS 本征核，在线重构都需要执行掩模衍射谱与本征核相乘、2D IFFT、幅度平方运算、特征值加权以及多核强度累加；这些操作会随本征核数量、掩模窗口数量和 OPC/SMO/ILT 迭代次数重复放大 [19]-[23]。在批量窗口评估、工艺窗口扫描或迭代式掩模优化中，SOCS 本征核、权重和控制配置可在同一光学条件下跨大量窗口复用，单次在线重构延迟和能耗的降低会在批处理吞吐中累积放大。因此，低功耗、确定性在线算子的工程价值不仅体现在单帧加速，也体现在批量掩模窗口流水线处理、边缘式线宽验证和系统级能效预算上。
...
这种划分方式源于两个阶段不同的执行特征。TCC 构建和特征分解需要高精度矩阵运算，但仅在光学参数变化时执行；相比之下，在线 SOCS 重构会针对不同掩模窗口重复执行，并且主要由规则的 FFT 和累加操作构成。因此，将在线阶段迁移到 FPGA 可将频繁执行的计算热点映射为确定性流水线，同时保留 CPU 在离线预处理方面的灵活性。
...
本节围绕频域嵌入、二维 IFFT 和存储访问三个决定性能与资源占用的环节概括本文的硬件设计。默认配置下，有效本征核尺寸为 $17 \times 17$，对应 $N_x=N_y=8$。嵌入模块以掩模衍射谱中心 $(L_x/2,L_y/2)$ 为基准，将中心窗口与本征核逐点复乘并映射到固定的 $128 \times 128$ 频率网格，其余位置补零，从而保持 FPGA 输出与 CPU 参考结果的频率排列和空间相位一致。嵌入循环采用 `PIPELINE II=1`，运行时参数 $N_x$ 和 $N_y$ 定义实际有效窗口，$17 \times 17$ 最大窗口作为综合边界。
...
**二维 IFFT 引擎。** 二维 IFFT 是主要计算阶段。本文的关键设计是在多个 SOCS 本征核之间时分复用单套 $128 \times 128$ 二维 IFFT 数据通路，并使用实部/虚部分离的双端口 BRAM 作为冲突规避转置缓冲区，避免行列变换之间的额外 DDR 往返。底层 128 点一维 FFT 采用自然序输出、块浮点缩放和 LUT 映射乘法；输入与输出使用 `ap_fixed<32,1>`。行列变换的缩放指数累加后，在定点到浮点转换阶段统一补偿：
...
该策略减少了固定逐级缩放的误差累积，并通过 LUT 映射蝶形乘法降低 DSP 使用量。本文由此将有限的 BRAM 优先分配给转置和累加缓冲，在不改变 SOCS 成像模型的前提下控制二维 IFFT 的面积开销。
...
为便于复现，表 3 给出了关键 HLS 与接口配置。外部 DDR 数据统一采用 IEEE-754 单精度浮点，FFT IP 内部采用 `ap_fixed<32,1>` 复数定点格式；块浮点模式通过 `blk_exp` 记录总缩放量，并按式 (9) 补偿回浮点结果。
...
进一步采用 10 个不同版图样本进行板上验证，所有样本均通过精度检查。FPGA+Host FI 相对同配置 SOCS software Golden 的平均 RMSE 为 $2.65\times10^{-8}$，最差样本 RMSE 为 $3.58\times10^{-8}$，平均 SSIM 为 0.999999995，阈值 0.225 下的二值图形一致率均为 100%。图 4 给出 10 个样本的掩模与板上重构结果对照。

![图 4 十个不同掩模样本的板上空中像重构结果](../image/论文/multi_mask_fpga_results.png)

**图 4** 十个不同掩模样本 T1--T10 的输入与 FPGA+Host FI 重构结果。上排为输入掩模，下排为逐列对应的空中像。
...
![图 6 性能、能效与资源总结](../image/论文/ch5_fig3_performance_resource.png)

**图 6** 所提出 FPGA/HLS 架构的性能、估算内核能效与资源总结。
...
本文提出一种面向 TCC-SOCS 空中像重构的低延迟高能效 FPGA 架构。该方法的核心思想是将光学系统相关的 TCC 构建与 SOCS 本征核提取保留在 CPU 端，将 OPC/SMO 等流程中频繁执行的在线重构阶段映射为 FPGA 上的专用数据通路，并通过 PCIe XDMA 将输入写入、AXI-Lite 控制、结果回读和主机侧傅里叶插值串接成完整执行路径。通过这种划分，复杂光学模型被压缩为可复用的本征核和权重，而 FPGA 只需执行频域嵌入、二维 IFFT、加权累加、FFTshift 和结果写回等规则计算，从而把 TCC-SOCS 的在线热点转化为可流水化、可部署的硬件任务。

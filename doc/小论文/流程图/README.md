...
- [04_CPU_FPGA_SOCS精简版.drawio](04_CPU_FPGA_SOCS精简版.drawio)：推荐用于投稿主图的精简版。仅保留频域输入、频域乘积/固定网格、单核 IFFT 强度、10 核加权累加和最终空中像五个可视化节点。图中的 `INSERT IMAGE` 子框可直接替换为对应结果图。
- [06_Linux主机出图交接.md](06_Linux主机出图交接.md)：Ubuntu + FPGA/XDMA 主机从 Golden、板上回读到统一 SCI 素材和回传包的完整操作手册。

精简版用于让审稿人直接看到三点：两条流程使用相同 SOCS 输入和数学模型；FPGA 将固定网格嵌入、二维 IFFT 和加权累加映射为流水线；相同 10 核在线重构从 `35.6 ms` 降低到 `10.57 ms`。

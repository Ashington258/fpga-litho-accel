...
在第 3 章中，我们通过 MATLAB 验证了"离线预计算 TCC + 在线 SOCS 重构"算法的可行性。然而，为了满足工业级 OPC（光学邻近效应修正）对大规模版图实时仿真的需求，单纯依赖通用处理器（CPU）的串行计算逻辑已难以突破计算密度与 I/O 带宽的瓶颈。

**核心加速目标**：
- **吞吐量提升**：实现单次 SOCS 空间像计算从 CPU 约 50ms（含 FFTW IFFT）降至 FPGA 约 1ms
- **时延优化**：支持流水线模式，实现连续 mask 数据的实时处理（目标吞吐率 ≥1000 mask/秒）
- **功耗控制**：相比 GPU 方案，FPGA 在相同算力下功耗降低约 60%（15W vs 40W）
...
| 计算延迟     | <10 ms   | 1.05 ms (@274MHz)         | ✅达标 |
| 相对精度误差 | <5%      | RMSE=9.55e-08 (0.39%)     | ✅达标 |
| 资源占用     | <50%     | BRAM 17%, DSP 2%, LUT 29% | ✅达标 |

### 4.1.2 软硬件任务划分

基于 SOCS 算法的计算特性，采用 **主机预处理 + FPGA 核心计算** 的异构协同架构：
...
├─────────────────────────────────────────────────────────────────────┤
│  Host CPU（预处理层）                                                │
│  ├─ JSON Parser：解析光学参数配置                                    │
│  ├─ Source Processor：生成光源分布矩阵（Annular/Dipole等模式）        │
│  ├─ Mask Processor：读取 mask 数据，计算 FFTW 2D FFT → mskf_r/i     │
│  ├─ TCC Calculator：计算传输交叉系数 TCC 矩阵                        │
│  ├─ Kernel Extractor：Eigen库特征值分解 → λ_k, K_k                  │
│  └─ Data Formatter：打包 FPGA 输入数据（BIN格式）                    │
├─────────────────────────────────────────────────────────────────────┤
│  FPGA（核心计算层）                                                  │
│  ├─ Mask Spectrum Extractor：提取 9×9 mask 频域区域                 │
│  ├─ Complex Multiplier Array：K_k × M 频域点乘                      │
│  ├─ Zero-Padding Engine：嵌入 32×32 padded 数组                     │
│  ├─ 2D IFFT Engine：Vitis HLS FFT IP（32×32 行-列分解）              │
│  ├─ Intensity Accumulator：累加 |IFFT|² × λ_k                       │
│  └─ FFTShift + Extractor：输出 17×17 有效区域                       │
└─────────────────────────────────────────────────────────────────────┘
```

**划分原则**：
1. **预计算任务上浮**：TCC、Kernel 计算涉及矩阵分解（O(N³)），适合 CPU 高精度浮点处理
2. **核心卷积下沉**：频域点乘 + IFFT 具有高度并行性，适合 FPGA 流水线加速
3. **数据格式统一**：采用 BIN 格式规范，确保主机-FPGA 数据一致性

...
def verify_trust_chain(config_path):
    ...
    for bin_name, bin_path in bin_files.items():
        ...
        success = run_verification_step(step_name, "verify_bin_hex_conversion.py", ["--input", bin_path])
    ...
    if not os.path.exists(golden_file):
        ...
        if not os.path.exists(ddr_read_file):
            ...
            # 运行环节3验证
            step_name = "HLS输出验证"
            success = run_verification_step(step_name, "verify_hls_output.py", 
...
def main():
    ...
    parser.add_argument("--config", default="board_validation_config.json", help="配置文件路径")

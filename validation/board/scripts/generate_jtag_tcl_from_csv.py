...
OUTPUT_DIR = Path("/root/project/FPGA-Litho/validation/board/scripts")
CSV_FILE = Path(
    "/root/project/FPGA-Litho/source/SOCS_HLS/socs_full_csynth_v13/hls/impl/ip/AddressSegments.csv"
)
...
def generate_write_transactions_for_file(
    bin_file, ddr_addr, chunk_size=MAX_WORDS_PER_TCL
):
    """
    ...
    print(
        f"[INFO] Processing {bin_file.name}: {total_words} floats ({total_words*4} bytes)"
    )
...
"""
    ...
    if mskf_r_file.exists():
        cmds, words, txns = generate_write_transactions_for_file(
            mskf_r_file, ddr_addrs["mskf_r"]
        )
    ...
    if kernels_dir.exists():
        ...
        for i in range(10):
            ...
            if krn_r_file.exists():
                ...
                tcl_content += (
                    f"# Writing krn_{i}_r.bin ({words} floats, {txns} transactions)\n"
                )
                tcl_content += "".join(cmds)
        ...
        for i in range(10):
            ...
            if krn_i_file.exists():
                ...
                tcl_content += (
                    f"# Writing krn_{i}_i.bin ({words} floats, {txns} transactions)\n"
                )
                tcl_content += "".join(cmds)
...
def main():
    ...
    print(
        "  2. Or batch mode: vivado -mode batch -source run_board_validation_auto.tcl"
    )

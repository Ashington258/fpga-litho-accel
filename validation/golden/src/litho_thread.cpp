 ...
 * Used for dynamic FFT sizing based on actual convolution requirements
 * ========================================================================
 */
...
void calcTCC(vector<ComplexD>& tcc, int tccSize, vector<float>& src, int srcSize, float outerSigma,
    ...
    // Compute TCC using row-parallel evaluation. Each thread computes one set of
    // upper-triangular rows, avoiding atomic updates on the shared matrix.
    int cols = 2 * Nx + 1;
    int rows = 2 * Ny + 1;
    int totalIDs = cols * rows;

    #pragma omp parallel for schedule(dynamic)
    for (int ID1 = 0; ID1 < totalIDs; ++ID1) {
        int ny = (ID1 / cols) - Ny;
        int nx = (ID1 % cols) - Nx;
        ...
        for (int q = -oSgm; q <= oSgm; q++) {
            for (int p = -oSgm; p <= oSgm; p++) {
                int srcID = (q + sh) * srcSize + p + sh;
                if (src[srcID] == 0.0f || (p * p + q * q) > sh * sh) continue;
                if (pupil[ID1][srcID] == ComplexD(0, 0)) continue;
                ...
                for (int my = ny; my <= nyMaxTmp; my++) {
                    int startmx = (my == ny) ? nx : nxMinTmp;
                    for (int mx = startmx; mx <= nxMaxTmp; mx++) {
                        if (my < -Ny || my > Ny || mx < -Nx || mx > Nx) continue;
                        int ID2 = (my + Ny) * cols + (mx + Nx);
...
void calcSOCS(vector<float>& image, const vector<ComplexD>& mskf,
    ...
    // Use the smallest power-of-two that fits the required convolution support.
    // This avoids truncation for larger Nx/Ny cases while still being FFT-friendly.
    ...
    // -----------------------------------------------------------------------
    // Create FFTW plan outside parallel region for thread safety
    vector<ComplexD> bwdDataInGlobal(fftConvX * fftConvY, ComplexD(0, 0));
    vector<ComplexD> bwdDataOutGlobal(fftConvX * fftConvY, ComplexD(0, 0));
    fftw_plan bwd_plan = fftw_plan_dft_2d(
        fftConvY, fftConvX,
        reinterpret_cast<fftw_complex*>(bwdDataInGlobal.data()),
        reinterpret_cast<fftw_complex*>(bwdDataOutGlobal.data()),
    ...
    #pragma omp parallel
    {
        ...
        vector<double> tmpImgPrivate(fftConvX * fftConvY, 0.0);
        ...
        for (int k = 0; k < nk; ++k) {
            ...
            // Use thread-safe execution with per-thread buffers
            fftw_execute_dft(bwd_plan,
            ...
            for (int i = 0; i < fftConvX * fftConvY; ++i) {
                ...
                tmpImgPrivate[i] += scales[k] * (re * re + im * im);
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    // Initialize FFTW threads for OpenMP parallel regions
    if (fftw_init_threads() == 0) {
        cerr << "Error: Failed to initialize FFTW threads" << endl;
        return 1;
    }
    
    // Set number of threads for FFTW to match OpenMP
    int num_threads = omp_get_max_threads();
    fftw_plan_with_nthreads(num_threads);
    cout << "[FFTW] Thread support initialized with " << num_threads << " threads" << endl;

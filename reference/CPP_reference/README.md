# Lithoo
Lithoo is a toolset for simulating optical lithography.

Lithoo includes two tools: Lithoo-TCC and Lithoo-SOCS.

- Lithoo-TCC: Calculates the optical image using the Transmission Cross Coefficient (TCC) and derives the Sum of Coherent Systems (SOCS) kernels. Lithoo-TCC allows you to set optical conditions such as mask transmittance, source shape and intensity data, NA, and defocus.

- Lithoo-SOCS: Calculates the optical image using the Sum of Coherent Systems (SOCS) kernels derived by Lithoo-TCC.

## Prerequisites

Before implementing Lithoo, make sure that [FFTW3](http://www.fftw.org/) and [LAPACK](http://www.netlib.org/lapack/) are installed.
...
Move to the Lithoo-TCC directory and compile using the make command.
```
cd Lithoo-TCC
make
```
...
A sample script for running Lithoo-TCC is provided in "test_tcc.sh". When running this script for the first time, grant execution permission with the following command:
```
chmod +x test_tcc.sh
```
...
You can use the kernel files created by Lithoo-TCC as input kernels.

### Compilation

Move to the Lithoo-SOCS directory and compile using the make command.
```
cd Lithoo-SOCS
make
```
...
You can specify simulation parameters when running the program. A sample script for running Lithoo-SOCS is provided in "test_socs.sh".

A sample script for running Lithoo-SOCS is provided in "test_socs.sh". When running this script for the first time, grant execution permission with the following command:
```

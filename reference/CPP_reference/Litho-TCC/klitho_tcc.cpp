...
void createOutputDir(const string& path) {
  struct stat st;
  if (stat(path.c_str(), &st) != 0) {
    ...
    cout << "Created output directory: " << path << endl;
  }
}

void ensureOutputDirs(const string& outputDir) {
  createOutputDir(outputDir);
  createOutputDir(outputDir + "/kernels");
  createOutputDir(outputDir + "/kernels/png");
}
...
void calcTCC(vector<Complex>& tcc, int tccSize, vector<float>& src, int srcSize, float outerSigma,
 float lambda, float NA, float defocus, int Lx, int Ly, int Nx, int Ny) {
  ...
  int nxMinTmp, nxMaxTmp, nyMinTmp, nyMaxTmp;
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 24) {
    cerr << "Error: Invalid number of arguments: " << argc << ". Expected 24 arguments." << endl;
    cerr << "Usage: " << argv[0] << " Lx Ly maskSizeX maskSizeY maskType lineWidth spaceWidth isHorizontal maskFile srcSize srcType innerRadius outerRadius radius offset onXAxis ptX ptY sourceFile NA defocus nk outputDir" << endl;
    return 1;
  }
  int Lx = stoi(argv[1]);
  int Ly = stoi(argv[2]);
  ...
  writeFloatArrayToPNGWithGrayscale(srcSize, srcSize, src, getMin(src), getMax(src), outputDir + "/source.png");
  ...
  writeFloatArrayToPNGWithGrayscale(Lx, Ly, msk, getMin(msk), getMax(msk), outputDir + "/mask.png");
...
outerSigma = 1;
  ...
  cout << endl << "------------------------------------" << endl;
  cout << "[MODULE 3] Calculating FFT Mask..." << endl;
  ...
  // Output FFT samples
  cout << "FFT Mask Samples (center region):" << endl;
  int fft_sample_count = 0;
  for (int y = Ly/2 - 2; y <= Ly/2 + 2 && fft_sample_count < 5; y++) {
    for (int x = Lx/2 - 2; x <= Lx/2 + 2 && fft_sample_count < 5; x++) {
      cout << "  mskf[" << y << "," << x << "] = (" << mskf[y*Lx + x].real() << ", " << mskf[y*Lx + x].imag() << ")" << endl;
      fft_sample_count++;
    }
  }
  ...
  cout << endl << "------------------------------------" << endl;
  cout << "[MODULE 4] Starting calcTCC..." << endl;
  ...
  max = getMax(tccReal);
  min = getMin(tccReal);
  double tccRealSum = 0.0, tccRealVar = 0.0;
  for (int i = 0; i < tccSize * tccSize; i++) {
    tccRealSum += tccReal[i];
  }
  double tccRealMean = tccRealSum / (tccSize * tccSize);
  for (int i = 0; i < tccSize * tccSize; i++) {
    tccRealVar += (tccReal[i] - tccRealMean) * (tccReal[i] - tccRealMean);
  }
  tccRealVar /= (tccSize * tccSize);
  
  cout << "TCC Matrix Statistics (Real):" << endl;
  cout << "  Max: " << max << endl;
  cout << "  Min: " << min << endl;
  cout << "  Mean: " << tccRealMean << endl;
  cout << "  StdDev: " << sqrt(tccRealVar) << endl;
  cout << "  Diagonal element [0]: " << tcc[0].real() << endl;
  cout << "  Center element [" << tccSize/2 << "]: " << tcc[tccSize/2 * tccSize + tccSize/2].real() << endl;
  cout << "TCC Matrix Samples (first 5 diagonal elements):" << endl;
  for (int i = 0; i < 5 && i < tccSize; i++) {
    cout << "  TCC[" << i << "," << i << "] = (" << tcc[i * tccSize + i].real() << ", " << tcc[i * tccSize + i].imag() << ")" << endl;
  }
  ...
  cout << endl << "------------------------------------" << endl;
  cout << "[MODULE 5] Starting calcImage..." << endl;
  ...
  max = getMax(img);
  min = getMin(img);
  double imgSum = 0.0, imgVar = 0.0;
  for (int i = 0; i < Lxy; i++) {
    imgSum += img[i];
  }
  double imgMean = imgSum / Lxy;
  for (int i = 0; i < Lxy; i++) {
    imgVar += (img[i] - imgMean) * (img[i] - imgMean);
  }
  imgVar /= Lxy;
  
  cout << "Aerial Image Statistics:" << endl;
  cout << "  Max: " << max << endl;
  cout << "  Min: " << min << endl;
  cout << "  Mean: " << imgMean << endl;
  cout << "  StdDev: " << sqrt(imgVar) << endl;
  cout << "Aerial Image Samples (center region):" << endl;
  int img_sample_count = 0;
  for (int y = Ly/2 - 2; y <= Ly/2 + 2 && img_sample_count < 5; y++) {
    for (int x = Lx/2 - 2; x <= Lx/2 + 2 && img_sample_count < 5; x++) {
      cout << "  img[" << y << "," << x << "] = " << img[y*Lx + x] << endl;
      img_sample_count++;
    }
  }

  writeFloatArrayToBinary(outputDir + "/image.bin", img, Lx*Ly);
  writeFloatArrayToPNGWithContinuousColor(Lx, Ly, img, outputDir + "/image.png", min, max);
  writeFloatArrayToPNGWith7Colors(Lx, Ly, img, outputDir + "/image_s.png", 0, TARGET_INTENSITY);

  long long t7_dur = 0;
  if (nk > 0) {
    // ===== MODULE 7: Calculate Kernels =====
    auto t7_start = system_clock::now();
    cout << endl << "------------------------------------" << endl;
    cout << "[MODULE 7] Starting calcKernels..." << endl;
    ...
    t7_dur = duration_cast<microseconds>(t7_end - t7_start).count();
    if (info != 0) {
  ...
  writeFloatArrayToPNGWithContinuousColor(2 * Nx + 1, 2 * Ny + 1, krnReal, outputDir + "/kernels/png/krn_" + to_string(i) + "_r.png", getMin(krnReal), getMax(krnReal));
  writeFloatArrayToPNGWithContinuousColor(2 * Nx + 1, 2 * Ny + 1, krnImag, outputDir + "/kernels/png/krn_" + to_string(i) + "_i.png", getMin(krnImag), getMax(krnImag));
}
writeKernelInfo(outputDir + "/kernels/kernel_info.txt", 2 * Nx + 1, 2 * Ny + 1, nk, scales);
  }
  ...
  cout << endl << "====================================" << endl;
  cout << "PERFORMANCE SUMMARY (Module Breakdown):" << endl;
  cout << "====================================" << endl;
  cout << "[Module 1] Create Source:  " << t1_dur * 1.0e-06 << " sec" << endl;
  cout << "[Module 2] Create Mask:    " << t2_dur * 1.0e-06 << " sec" << endl;
  cout << "[Module 3] FFT Mask:       " << t3_dur * 1.0e-06 << " sec" << endl;
  cout << "[Module 4] calcTCC:        " << t4_dur * 1.0e-06 << " sec" << endl;
  cout << "[Module 5] calcImage:      " << t5_dur * 1.0e-06 << " sec" << endl;
  cout << "[Module 6] Inverse FFT:    " << t6_dur * 1.0e-06 << " sec" << endl;
  cout << "[Module 7] calcKernels:    " << t7_dur * 1.0e-06 << " sec" << endl;
  cout << "------------------------------------" << endl;
  ...
  cout << endl << "====================================" << endl;
  cout << "VERIFICATION SUMMARY (For HLS C-Sim):" << endl;
  cout << "====================================" << endl;
  cout << "Key values to compare with HLS output:" << endl;
  cout << "  - TCC Mean (Real): " << tccRealMean << endl;
  cout << "  - TCC StdDev (Real): " << sqrt(tccRealVar) << endl;
  cout << "  - Image Mean: " << imgMean << endl;
  cout << "  - Image StdDev: " << sqrt(imgVar) << endl;

/**
 * @file tb_calc_tcc_correct.cpp
 * @brief calcTCC模块测试平台 - Phase 1验证（修正版）
 * 
 * 测试目标：
 * - 验证calc_tcc模块与CPU参考的一致性
 * - 统计验证：相对误差 < 1e-5
...
bool verify_tcc(const vector<cmpxData_t>& hls_tcc, const vector<cmpxData_t>& golden_tcc,
    ...
    if (abs(golden_mean_i) > 1e-10) {
        cout << "  Relative Error (Mean): " << abs((hls_mean_i - golden_mean_i) / golden_mean_i) << endl;
    } else {
        cout << "  Relative Error (Mean): N/A (golden mean near zero)" << endl;
    }
    cout << endl;
    cout << "  HLS StdDev:   " << hls_std_i << endl;
    cout << "  Golden StdDev: " << golden_std_i << endl;
    if (abs(golden_std_i) > 1e-10) {
        cout << "  Relative Error (StdDev): " << abs((hls_std_i - golden_std_i) / golden_std_i) << endl;
    } else {
        cout << "  Relative Error (StdDev): N/A (golden stddev near zero)" << endl;
    }
    cout << endl;
...
int main() {
    ...
    // 1.1 加载光源数据
    vector<data_t> src;
    if (!load_binary_file("golden/source_101x101.bin", src, SRC_SIZE * SRC_SIZE)) {
    ...
    int oSgm = (int)ceil(sh * OUTER_SIGMA);  // 光源有效半径 = 45

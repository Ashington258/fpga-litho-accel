...
### 资源占用 (xcku5p-ffvb676-2-e)

| 资源类型 | 使用量 | 可用量 | 占用率 | 状态 |
|---------|--------|--------|--------|------|
| BRAM_18K | 399 | 960 | **42%** | ✅ 充足 |
| DSP | 53 | 1,824 | 3% | ✅ 充足 |
| FF | 31,942 | 433,920 | 7% | ✅ 充足 |
| LUT | 37,098 | 216,960 | 17% | ✅ 充足 |
...
3. **Accumulate依赖**:
   - tmpImg需要累积所有kernel结果
   - DATAFLOW要求stage间无依赖
   - 无法实现完全并行

#### ✅ 替代优化方向

**Phase 1: 提升Fmax** (推荐立即实施)
- 当前: 274 MHz
- 目标: 300 MHz
- 提升: 1.1倍吞吐量
- 风险: 低
...
- 收益: BRAM节省20-30%，DSP节省30%
- 风险: 中等（需验证精度）
- 实施时间: 2-3小时

**Phase 3: 有限并行化** (中期，前提是Phase 2成功)
- 策略: UNROLL factor=2，仅并行embed阶段
- 收益: 1.5-1.8倍吞吐量
- 风险: 高（资源可能超限）
- 实施时间: 4-6小时
...
参见: `doc/PARALLEL_DESIGN.md`

**下一步行动**:
1. ✅ 完成并行化可行性分析
2. ⏳ 实施Phase 1: 提升Fmax到300MHz
3. ⏳ 验证时序收敛
4. ⏳ 如果成功，继续Phase 2精度优化

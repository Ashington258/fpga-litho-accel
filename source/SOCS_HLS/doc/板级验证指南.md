...
**数据路径**: `e:\fpga-litho-accel\output\verification\`
...
创建文件 `validation/board/scripts/generate_jtag_tcl.py`:
...
if __name__ == '__main__':
    output_tcl = 'validation/board/scripts/run_board_validation.tcl'
    generate_full_tcl_script(output_tcl)
```

### 5.2 输出数据解析与Golden对比

创建文件 `validation/board/scripts/compare_results.py`:
...
def main():
    ;# 文件路径
    board_output_file = 'validation/board/board_output.txt'
    golden_file = '/root/project/FPGA-Litho/output/verification/tmpImgp_pad32.bin'
    output_prefix = 'validation/board/results/validation'
...
mkdir -p validation/board/scripts validation/board/results

# 3. 生成TCL脚本
python validation/board/scripts/generate_jtag_tcl.py
```
...
vivado -mode batch -source validation/board/scripts/run_board_validation.tcl

# 或在Vivado GUI中：
# Tools -> Open Hardware Manager
# 在Tcl Console中执行：
# source validation/board/scripts/run_board_validation.tcl
```
...
```bash
# 解析输出并对比Golden
python validation/board/scripts/compare_results.py

# 查看结果
cat validation/board/results/validation_metrics.txt
ls validation/board/results/*.png
```
...
**文件**: `validation/board/scripts/run_board_validation_template.tcl`

...
# 9. 地址分配（⚠️ 必须使用脚本修复重叠）
# 问题：Vivado 自动分配会将所有未配置的 m_axi 接口分配到相同地址
# 解决方案：使用脚本或手动在 Address Editor 中分配

# 方案 A：使用 TCL 脚本自动修复（推荐）
source vivado_bd/fix_hls_address_overlap.tcl
fix_hls_ip_address_overlap $IP_NAME

# 方案 B：手动在 Address Editor 中分配（见下文推荐地址表）
...
当 HLS IP 导入 Vivado Block Design 时，多个 `m_axi` 接口（gmem0-gmem5）会被自动分配到相同的地址段（通常为 `0x40000000`），导致 Address Editor 显示 "Overlapping Segments" 和 "Excluded" 状态。

**根本原因**：Vivado 自动分配逻辑将所有未配置的 AXI-Master 接口默认分配到同一起始地址。

**解决方案**：使用 `vivado_bd/fix_hls_address_overlap.tcl` 脚本自动修复，或按以下表格手动分配：

#### 推荐的非重叠地址分配表

| AXI 接口         | 数据类型 | 元素数量 | 数据大小 | 推荐基地址   | 地址范围                  |
| ---------------- | -------- | -------- | -------- | ------------ | ------------------------- |
| `gmem0` (mskf_r) | float    | 262144   | 1 MB     | `0x40000000` | `0x40000000 - 0x400FFFFF` |
| `gmem1` (mskf_i) | float    | 262144   | 1 MB     | `0x40100000` | `0x40100000 - 0x401FFFFF` |
| `gmem2` (scales) | float    | 10       | 40 B     | `0x40200000` | `0x40200000 - 0x4020003F` |
| `gmem3` (krn_r)  | float    | 810      | 3.24 KB  | `0x40200100` | `0x40200100 - 0x40200FFF` |
| `gmem4` (krn_i)  | float    | 810      | 3.24 KB  | `0x40201000` | `0x40201000 - 0x40201FFF` |
| `gmem5` (output) | float    | 289      | 1.16 KB  | `0x40202000` | `0x40202000 - 0x40202FFF` |
| `s_axi_control`  | Control  | -        | 16 B     | `0x40203000` | `0x40203000 - 0x4020300F` |

**注意**：
- 元素数量对应 HLS pragma 中的 `depth` 参数
- 地址段之间保留 64B 对齐间隔，避免边界重叠
- 总地址空间约 2.01 MB（远小于典型 DDR 容量）
...
// 定义基地址（基于推荐地址分配表）
#define SOC_HLS_CTRL_BASE   0x40203000  // s_axi_control 控制接口

// 定义数据缓冲区地址（对应 gmem0-gmem5 接口）
#define MSKF_R_ADDR   0x40000000  // gmem0: 1MB (262144 elements)
#define MSKF_I_ADDR   0x40100000  // gmem1: 1MB (262144 elements)
#define SCALES_ADDR   0x40200000  // gmem2: 40B (10 elements)
#define KRN_R_ADDR    0x40200100  // gmem3: 3.24KB (810 elements)
#define KRN_I_ADDR    0x40201000  // gmem4: 3.24KB (810 elements)
#define OUTPUT_ADDR   0x40202000  // gmem5: 1.16KB (289 elements)

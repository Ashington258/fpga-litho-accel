/**
 * @file socs_2048_stream_refactored_v6.cpp
 * @brief SOCS HLS implementation with Multi-FFT Instance Parallelization + DDR Optimization
 * 
 * Phase 3优化：多FFT实例并行（UNROLL factor=2）+ DDR优化
 * 
 * v6优化内容：
 * - 将tmpImg_final移至DDR外部存储
 * - 新增AXI-MM接口（gmem6）用于tmpImg_final
 * - 节省~200 BRAM
 * - 使用AXI burst读写优化DDR访问
 * 
 * 设计目标：
 * - 使用UNROLL factor=2实现2个kernel并行处理
 * - tmpImg_final存储在DDR，避免BRAM超限
 * - 预期：BRAM占用率降至~75%
 * 
 * 架构：
 * - kernel_loop: UNROLL factor=2（2个kernel并行）
 * - 每个并行实例：embed → FFT → accumulate_to_DDR
 * - 输出阶段：从DDR读取 → FFTshift → output
 */
...
void fftshift_and_output_from_ddr(
    ...
    // Read from DDR to local array
    for (int y = 0; y < MAX_FFT_Y; y++) {
        for (int x = 0; x < MAX_FFT_X; x++) {
            #pragma HLS PIPELINE II=1
            int idx = y * MAX_FFT_X + x;
            tmpImgp[y][x] = tmpImg_ddr[idx];
        }
    }
    ...
    float tmpImg_shifted[MAX_FFT_Y][MAX_FFT_X];
    #pragma HLS RESOURCE variable=tmpImg_shifted core=RAM_2P_BRAM
    ...
    for (int y = 0; y < MAX_FFT_Y; y++) {
        for (int x = 0; x < MAX_FFT_X; x++) {
            ...
            tmpImg_shifted[y][x] = tmpImgp[src_y][src_x];
        }
    }
    ...
    for (int y = 0; y < MAX_FFT_Y; y++) {
        for (int x = 0; x < MAX_FFT_X; x++) {
            ...
            output[out_idx] = tmpImg_shifted[y][x];
        }
    }
}
...
void calc_socs_2048_hls_stream_refactored(
    ...
    kernel_pair_loop: for (k = 0; k < nk - 1; k += 2) {
        ...
        // Parallel instance 0: kernel k
        {
            ...
            accumulate_to_ddr(fft_out_stream_0, tmpImg_ddr, scales[k]);
        }
        ...
        // Parallel instance 1: kernel k+1
        {
            ...
            accumulate_to_ddr(fft_out_stream_1, tmpImg_ddr, scales[k+1]);
        }
    }
    ...
    if (k < nk) {
        ...
        accumulate_to_ddr(fft_out_stream_odd, tmpImg_ddr, scales[k]);
    }

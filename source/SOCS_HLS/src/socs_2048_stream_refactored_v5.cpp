/**
 * @file socs_2048_stream_refactored_v5.cpp
 * @brief SOCS HLS implementation with Multi-FFT Instance Parallelization + BRAM Optimization
 * 
 * Phase 3优化：多FFT实例并行（UNROLL factor=2）+ BRAM优化
 * 
 * v5优化内容：
 * - 移除tmpImg_local数组，直接累加到tmpImg_final
 * - 节省400 BRAM（35%）
 * - 使用ARRAY_PARTITION factor=8支持并行访问
 * - 保持DATAFLOW流水线重叠性能
 * 
 * 设计目标：
 * - 使用UNROLL factor=2实现2个kernel并行处理
 * - 直接累加到tmpImg_final，避免中间存储
 * - 预期：BRAM占用率降至~75%
 * 
 * 架构：
 * - kernel_loop: UNROLL factor=2（2个kernel并行）
 * - 每个并行实例：embed → FFT → accumulate_to_final（直接累加）
...
// ============================================================================
// Stage 3: Accumulate intensity directly to final array (BRAM optimized)
// ============================================================================

void accumulate_to_final(
    ...
    for (int y = 0; y < MAX_FFT_Y; y++) {
        for (int x = 0; x < MAX_FFT_X; x++) {
            ...
            // Direct accumulation to final array (no local storage)
            tmpImg_final[y][x] += scale * intensity;
        }
    }
}
...
void calc_socs_2048_hls_stream_refactored(
    ...
    // Final accumulator array (optimized with ARRAY_PARTITION for parallel access)
    float tmpImg_final[MAX_FFT_Y][MAX_FFT_X];
    
    #pragma HLS RESOURCE variable=tmpImg_final core=RAM_2P_BRAM
    #pragma HLS ARRAY_PARTITION variable=tmpImg_final cyclic factor=8 dim=2
    ...
    kernel_pair_loop: for (k = 0; k < nk - 1; k += 2) {
        ...
        // Parallel instance 0: kernel k
        {
            ...
            accumulate_to_final(fft_out_stream_0, tmpImg_final, scales[k]);
        }
        ...
        // Parallel instance 1: kernel k+1
        {
            ...
            accumulate_to_final(fft_out_stream_1, tmpImg_final, scales[k+1]);
        }
    }
    ...
    if (k < nk) {
        ...
        accumulate_to_final(fft_out_stream_odd, tmpImg_final, scales[k]);
    }

/**
 * @file socs_2048_stream_refactored_v7.cpp
 * @brief SOCS HLS implementation with Multi-FFT Instance Parallelization + DDR-based FFTshift
 * 
 * Phase 3优化：多FFT实例并行（UNROLL factor=2）+ DDR-based FFTshift
 * 
 * v7优化内容（方案E）：
 * - 移除所有临时BRAM数组（tmpImgp, tmpImg_shifted）
 * - 直接在DDR上进行FFTshift操作
 * - 使用streaming方式处理数据
 * - 预期节省~200 BRAM
 * 

 ...
 * - Compensation: output * 2^(blk_exp)
 */
void fft_2d_full_2048_hls_fft_v17(
    cmpx_2048_t input[MAX_FFT_Y][MAX_FFT_X],
    cmpx_2048_t output[MAX_FFT_Y][MAX_FFT_X],
    bool is_inverse
) {
    // v17: REMOVED DATAFLOW to avoid nested DATAFLOW issue
    // DATAFLOW is now at kernel_loop level
...
void calc_socs_2048_hls_stream_refactored_v17(
    ...
    // Process each kernel
    // v17: Removed DATAFLOW (requires compile-time trip count)
    // Using sequential execution for Co-Simulation compatibility
    kernel_loop:
    for (int k = 0; k < nk; k++) {
        ...
        // Stage 2: 2D IFFT using HLS FFT IP (v17: Removed nested DATAFLOW)
        fft_2d_full_2048_hls_fft_v17(
            fft_input,
            fft_output,

/**
 * @file tb_socs_1024_stream_refactored_v16_save_output.cpp
 * @brief Testbench for SOCS 2048 Stream Refactored v16 with output saving
 */
...
int write_binary_file(const char* filename, float* data, int size) {
    ...
    int num_written = fwrite(data, 1, size, fp);
    fclose(fp);
    
    printf("[INFO] Written %d bytes to %s\n", num_written, filename);
    return 0;
}
...
int main() {
    ...
    printf("HLS FFT IP with Block Floating Point Mode\n");
    printf("====================================================\n");
    ...
    printf("  Mode: Block Floating Point (maximum precision)\n");
    printf("====================================================\n\n");
    ...
    printf("Step 1: Loading input data...\n");
    
    std::string base_path = "/home/ashington/project/optimization/fpga-litho-accel/output/verification/";
    ...
    printf("Step 3: Running calc_socs_2048_hls_stream_refactored_v16...\n");
    ...
    printf("Step 4: Saving output to file...\n");
    
    std::string output_path = "/home/ashington/project/optimization/fpga-litho-accel/output/verification/tmpImgp_v16_output.bin";
    if (write_binary_file(output_path.c_str(), output, MAX_FFT_X * MAX_FFT_Y * sizeof(float)) < 0) {
        printf("[ERROR] Failed to save output\n");
        return 1;
    }
    
    printf("[INFO] Output saved to: %s\n\n", output_path.c_str());
    
    // Step 5: Validate results
    printf("Step 5: Validating results against golden output...\n");

/**
 * @file tb_socs_1024_stream_refactored_v7.cpp
 * @brief Testbench for SOCS 2048 Stream Refactored v7 (Phase 3 - DDR-based FFTshift)
 * 
 * Validates the DDR-based FFTshift optimization (no BRAM arrays) against golden_1024 data.
 */
...
int main() {
    ...
    printf("Step 3: Running calc_socs_2048_hls_stream_refactored (v6 - DDR Optimized)...\n");
    printf("[INFO] tmpImg_final stored in DDR (saves ~200 BRAM)\n");
    ...
    printf("Step 4: Comparing HLS output with golden reference...\n");
    ...
    if (rmse < tolerance) {
        ...
        printf("  - BRAM: Reduced by ~200 (tmpImg_final moved to DDR)\n");
        printf("  - Throughput: ~2x (2 kernels processed simultaneously)\n");
        printf("  - DDR access: Sequential pattern, burst-friendly\n");
        printf("====================================================\n");
        return 0;
    } else {

/*
 * SOCS测试平台（AXI-Lite接口版本）
 * FPGA-Litho Project
 * 
 * 测试目标：验证socs_top_fixed接口的正确性
 */
...
int main() {
    cout << "\n========================================" << endl;
    cout << "SOCS Test (AXI-Lite Interface Version)" << endl;
    cout << "========================================\n" << endl;
    ...
    for (unsigned int k = 0; k < nk; k++) {
        ...
        for (unsigned int i = 0; i < krnSize; i++) {
    ...
    for (unsigned int i = 0; i < Lx * Ly; i++) hls_mskf[i] = mskf_data[i];
    for (unsigned int i = 0; i < nk * krnSize; i++) hls_krns[i] = krns_data[i];
    for (unsigned int i = 0; i < nk; i++) hls_scales[i] = scales_data[i];
    
    cout << "  Input prepared" << endl;
    
    // 执行HLS函数（带参数）
    cout << "\n[3] Running HLS function with AXI-Lite parameters..." << endl;
    socs_top_fixed(hls_mskf, hls_krns, hls_scales, hls_image, Lx, Ly, Nx, Ny, nk);
    cout << "  Completed" << endl;
